...
RSpec.describe Relaton::Ietf::DataFetcher do
  ...
  context "instance ietf-internet-drafts" do
    ...
    it "update versions" do
      ...
      expect(Relaton::Ietf::ItemData).to receive(:new).with(formattedref: "draft-collins-pfr-01", docidentifier: [:id]).and_return(:bibitem)
      ...
      subject.send(:update_versions, ["draft-collins-pfr-01", "draft-collins-pfr1-02"])
    end
    ...
    it "create unversioned doc" do
      ...
      expect(Relaton::Ietf::ItemData).to receive(:new).with(formattedref: "draft-collins-pfr-00", docidentifier: [:id1]).and_return(:bibitem1)
      expect(Relaton::Ietf::ItemData).to receive(:new).with(formattedref: "draft-collins-pfr-01", docidentifier: [:id2]).and_return(:bibitem2)
      ...
      subject.send(:create_series, "draft-collins-pfr", ["draft-collins-pfr-00", "draft-collins-pfr-01"])
    end

    it "create version relation" do
      rel = subject.send(:version_relation, "draft-collins-pfr-00", "includes")
  ...
  context "save doc" do
    ...
    it "downcase file name for ID" do
      ...
      docid = double("docid", type: "Internet-Draft", content: "I-D.3gpp-collaboration")
      id_entry = double("entry", docidentifier: [docid])
      expect(subject.send(:file_name, id_entry)).to eq "dir/i-d.3gpp-collaboration.xml"
    end
  end
end
